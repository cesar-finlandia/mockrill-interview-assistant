...
// Patch: support -e <code> for handout verification (vite-node 6 removed it)
{
  const eIdx = process.argv.indexOf("-e");
  if (eIdx !== -1) {
    const code = process.argv[eIdx + 1];
    if (!code) { console.error("missing -e value"); process.exit(2); }
    const tmp = path.join(os.tmpdir(), `vite-node-e-${Date.now()}-${Math.random().toString(36).slice(2)}.mts`);
    fs.writeFileSync(tmp, code, "utf8");
    process.argv.splice(eIdx, 2, tmp);
  }
}
//#region package.json
var version$1 = "6.0.0";
