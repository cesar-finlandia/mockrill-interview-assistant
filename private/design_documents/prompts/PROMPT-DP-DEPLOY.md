# PROMPT-DP-DEPLOY — author the design plan for deployment and the hosted URL
...
`private/design_documents/design_plans/DP-DEPLOY.md`.

## YOUR PLAN: DP-DEPLOY — Vercel Deployment, Routes & Health Smoke

Submission field 10 is a **hosted Application URL required for interactive
evaluation**, and `getUserMedia` only works on a secure origin. This plan is
therefore a submission-gate plan: if it fails, the entry cannot be judged
interactively no matter how good the code is.

### What your plan must own (rows M40, M41, and the `build`/`preview` scripts)

**M41 `vercel.json` at the repo root.** The chassis ships
`config/deploy/vercel.json` (framework `vite`, `buildCommand: npm run build`,
`outputDirectory: dist`, `installCommand: npm ci`, SSE headers on `/events`).
Vercel reads the file at the **repo root**, so your plan creates a root
`vercel.json` that carries those settings plus:
- a rewrite `{"source": "/health", "destination": "/api/health"}` — the chassis
  smoke script polls `GET $PUBLIC_URL/health`, but Vercel serverless functions
  live under `/api/*`, so without this rewrite `deploy:verify` fails on a
  perfectly good deployment. **Call this out as a known trap.**
- `Cache-Control: no-store` on `/api/aai-token`.
- `functions` runtime pinned to a Node 20+ runtime.
State clearly that `config/deploy/vercel.json` is chassis and must not be edited;
the root file is new and additive.

**M40 `api/health.ts`.** `GET /api/health` → `200 { ok: true, version, commit }`
where `version` comes from `package.json` and `commit` from
`process.env.VERCEL_GIT_COMMIT_SHA ?? "local"`. No auth, no key access, no
dependency on any other module — it must return 200 even if every other part of
the app is broken, because it is the smoke target.

**`package.json` scripts you own (and only these):**
- `"build": "vite build"` — required because `config/deploy/vercel.json` sets
  `buildCommand: npm run build` while the chassis only defines `build:ui`.
  **This missing script is a real, present defect in the working copy** — say so.
- `"preview": "vite preview --port 4173"`.

### Environment and secrets (NFR-04)

Write the complete table: variable · where it is set · who reads it · what
happens when it is missing.

| var | set in | read by | missing → |
|---|---|---|---|
| `ASSEMBLYAI_API_KEY` | Vercel project env (Production + Preview) and local `.env` | `api/aai-token.ts`, `api/turn.ts` only | token endpoint returns 503 degraded; app falls to golden replay |
| `MOCKRILL_LLM_MODEL` | optional | `api/turn.ts` | defaults to `claude-sonnet-4-6` |
| `MOCKRILL_LLM_FALLBACK_MODEL` | optional | `api/turn.ts` | defaults to `qwen3.5-4b-32k-fast` |
| `RES_FORCED_DEGRADED` | set to `1` only for a rung-2 demo | `src/resilience` | unset = live behavior |
| `PUBLIC_URL` | local `.env` after deploy | `scripts/smoke-deploy.ts` | smoke polls `http://localhost:3000` |
| `VERCEL_TOKEN`, `VERCEL_PROJECT_ID` | local `.env` | `npm run deploy` | deploy fails with a clear message |
| `THEME`, `TRANSPORT`, `API_BASE`, `DEPLOY_PROVIDER` | optional | chassis `src/platform/config/env.ts` | documented chassis defaults |

Your plan adds `ASSEMBLYAI_API_KEY`, `MOCKRILL_LLM_MODEL` and
`MOCKRILL_LLM_FALLBACK_MODEL` to a **new root `.env.example`** — state that
`config/env.example` is chassis and read-only, so the additions go in the new
root file, and that `.env` is already git-ignored.

### The deploy runbook (numbered, copy-pasteable)

Write it out exactly:
1. `npm ci`
2. `npm run build` → expected `dist/` output line
3. `npx vercel link` (once), set the three env vars in the Vercel dashboard
4. `npm run deploy` → prints `[deploy] OK vercel <url> in <ms>ms`
5. Put that URL into `.env` as `PUBLIC_URL`
6. `npm run deploy:verify` → expected `PASS <ms>ms (attempt N, status 200)`
7. Open the URL, confirm the browser grants the mic on HTTPS, run one 30-second
   session, and confirm the scorecard renders.

Also specify the rollback path (`runRollback` via `npx vercel rollback`) and the
fallback providers already present in `src/platform/deploy/adapters/`
(`replit`, `docker`) with the one-line `DEPLOY_PROVIDER` switch.

### Non-negotiables

- **Audio never proxies through a serverless function.** If a later plan proposes
  a WebSocket relay in `api/`, reject it: Vercel functions cannot hold long-lived
  sockets and the design is browser → AssemblyAI direct.
- Exactly three functions ship: `api/aai-token.ts`, `api/turn.ts`,
  `api/health.ts`. Adding a fourth needs a blueprint amendment.
- No secret is ever written into `vercel.json`, `.env.example`, a fixture, or a
  log line.
- Deploy by the **end of week 2**, not week 4. A deployment that has never been
  smoke-tested is not a deployment.

### Mandatory work units (at least these)

1. `WU-DEP-01` — add `build` and `preview` scripts; verify `npm run build` succeeds.
2. `WU-DEP-02` — `api/health.ts` (M40); verify locally with `curl` and the exact expected JSON.
3. `WU-DEP-03` — root `vercel.json` (M41) with the `/health` rewrite; verify the rewrite resolves after deploy.
4. `WU-DEP-04` — root `.env.example` and the env table.
5. `WU-DEP-05` — first real deploy + `deploy:verify`, with the exact expected `PASS` line.
6. `WU-DEP-06` — HTTPS microphone verification on the deployed URL, with a written pass/fail checklist.
7. `WU-DEP-07` — rollback and alternate-provider runbook.
