# PROMPT-DP-AAI-STREAM — author the design plan for the AssemblyAI streaming client
...
`private/design_documents/design_plans/DP-AAI-STREAM.md`.

## YOUR PLAN: DP-AAI-STREAM — AssemblyAI Universal-Streaming Client & Token Endpoint

This plan delivers the mandatory-technology core of the entry: microphone
capture, the temporary-token endpoint, and the browser WebSocket client that
talks to `wss://streaming.assemblyai.com/v3/ws`. If this plan is wrong the entry
is disqualified, so every parameter must be written out literally from §S5.

### What your plan must own (rows M16–M20)

**M16 `api/aai-token.ts`** — a Vercel serverless function.
- Method `GET`. Rejects any other method with 405.
- Reads `process.env.ASSEMBLYAI_API_KEY`. **This is the only place besides
  `api/turn.ts` where the key may be read.** If absent, respond `503` with a
  `DegradedResult` body (`makeDegradedResult({ reason: "aai_key_missing",
  fallback_source: "none" })`) — never throw, never echo the key.
- Calls `GET https://streaming.assemblyai.com/v3/token?expires_in_seconds=60&max_session_duration_seconds=1800`
  with header `Authorization: <key>`, the whole call wrapped in `withResilience`
  with `{ timeout_ms: 15000, retries: 1, fallback_chain: { order: ["cache","none"] } }`.
- On success respond `200 { token, expires_in_seconds }` with
  `Cache-Control: no-store`.
- On a degraded result respond `503` with the `DegradedResult` JSON verbatim.
- Specify the 1800 s (30 min) session cap as a deliberate credit-protection
  choice and say why (billing is on socket-open duration).

**M17 `src/mockrill/voice/mic.ts` → `createMicSource`.** Spell out completely:
...
2. `new AudioContext({ sampleRate: 16000 })`; state the fallback when the browser
   refuses 16000 (create at the native rate and downsample in the worklet —
   give the linear-interpolation resample algorithm as numbered steps).
3. An `AudioWorkletProcessor` registered from a **blob URL built from an inline
   source string** (no separate asset file, so the bundle stays self-contained).
   Give the full worklet source in the plan.
4. The worklet buffers Float32 frames until it holds `3200` samples (= 200 ms at
   16 kHz), converts to `Int16Array` with `Math.max(-1, Math.min(1, s)) * 0x7FFF`,
   and posts it to the main thread.
5. `setMuted(true)` must **drop chunks without closing the socket** (the socket
   keeps its billing clock either way, but dropping audio is what prevents the
   agent from transcribing its own TTS output).
6. `stop()` disconnects the worklet, closes the `AudioContext`, and stops every
   track on the `MediaStream`.
7. Permission denial is not an exception to the caller: `createMicSource`
   rejects with a typed error whose `.name` is `"MicPermissionDenied"`, and the
   caller (DP-TURNTAKING) turns that into an envelope.

**M18/M19 `src/mockrill/voice/streamingClient.ts`.** Spell out:
1. `connect()` first `fetch`es `opts.tokenUrl ?? "/api/aai-token"`; a non-200
   response returns a `DegradedResult` (reason `"aai_token_unavailable"`) rather
   than throwing.
2. The socket URL is built by appending, in this exact order:
   `token`, `speech_model` (default `universal-3-5-pro`), `sample_rate=16000`,
   `encoding=pcm_s16le`, `format_turns=true`, `end_of_turn_confidence_threshold=0.45`,
   `min_turn_silence=560`, `max_turn_silence=1536`, `vad_threshold=0.2`,
   `interruption_delay=300`, `mode=balanced`, and `keyterms_prompt` (repeated
   once per keyterm) when `opts.keyterms` is non-empty. Justify each non-default
   value in one sentence each — these choices are Application-of-Technology
   evidence and DP-PITCH will quote them.
3. Message handling: `Begin` → `onBegin(id)`; `Turn` → map the AssemblyAI payload
   to `TranscriptTurn` (M2) with `speaker: "candidate"` and
   `received_at: new Date().toISOString()`, then route to `onPartial` when
   `end_of_turn === false` and to `onFinal` when
   `end_of_turn === true && turn_is_formatted === true`. State explicitly that a
   `Turn` with `end_of_turn === true && turn_is_formatted === false` is delivered
   to `onPartial` and **not** treated as final, so the formatted version is the
   one that gets scored.
4. `sendAudio(pcm)` sends `pcm.buffer` as a binary frame only when
   `state === "open"`; otherwise it drops the chunk silently and increments an
   internal `droppedChunks` counter that the plan exposes for diagnostics.
5. `updateConfiguration(patch)` sends `{ type: "UpdateConfiguration", ...patch }`.
6. `forceEndpoint()` sends `{ type: "ForceEndpoint" }`.
7. `terminate()` sends `{ type: "Terminate" }`, waits up to 2000 ms for the
   `Termination` message, then closes the socket regardless. State that
   `terminate()` MUST be called on unmount/`beforeunload` because billing runs on
   socket-open time.
8. Reconnection policy: exactly **one** automatic reconnect attempt after an
   unexpected close, with a fresh token, after a 1000 ms delay. A second failure
   calls `onDegraded` with reason `"aai_socket_lost"` and leaves the client
   `closed`. No infinite retry loops.
9. `RES_FORCED_DEGRADED=1` / golden-cache path: `connect()` must consult the
   wrapped call's result and, when it is a `DegradedResult` carrying cached data,
   invoke `onFinal` for each cached turn instead of opening a socket. Specify the
   cache key convention `mockrill-session-<sessionId>` and note that
   DP-DEMOPROOF records those entries.

**M20 `src/mockrill/voice/index.ts`** — the barrel. It re-exports M17–M19 and
**also** M21–M22, which are owned by DP-TURNTAKING. Say this explicitly: the
barrel file is yours, the two exports it forwards are not, and DP-TURNTAKING
creates those files. Your plan's barrel work unit therefore depends on
DP-TURNTAKING's files existing, and must be sequenced last.

### Non-negotiables to state in the plan

- **Audio never proxies through a serverless function.** Vercel cannot hold long
  sockets; browser → AssemblyAI is the only permitted audio path.
- The raw `ASSEMBLYAI_API_KEY` must never appear in client code, in a query
  string sent from the browser, or in any bundled file.
- No AssemblyAI SDK dependency: use the browser's native `WebSocket` and `fetch`.
  Adding an npm dependency for this is out of scope.

### Mandatory work units (at least these)

1. `WU-AAI-01` — `api/aai-token.ts` (M16). Verification: `curl -s -o /dev/null -w "%{http_code}" http://localhost:4173/api/aai-token` and a documented expected `200`, plus the documented `503` shape when the key is unset.
2. `WU-AAI-02` — the PCM worklet + `createMicSource` (M17).
3. `WU-AAI-03` — the socket URL builder as a **pure exported-for-test function**, verified by asserting the exact query string for a fixed input.
4. `WU-AAI-04` — message mapping `Turn` → `TranscriptTurn`, verified against a literal AssemblyAI `Turn` JSON fixture (write the fixture into the plan) and asserting the resulting `TranscriptTurn` field by field, importing the real `TranscriptTurn` type from `src/mockrill/contracts`.
5. `WU-AAI-05` — connect / reconnect / terminate lifecycle.
6. `WU-AAI-06` — degraded + golden-cache replay path.
7. `WU-AAI-07` — the barrel (M20), sequenced after DP-TURNTAKING.
