# PROMPT-DP-PITCH — author the design plan for the deck, script, Q&A and demo capture
...
`private/design_documents/design_plans/DP-PITCH.md`.

## YOUR PLAN: DP-PITCH — Deck (PDF), Timed Script, Judge Q&A & DEMODRIVE Capture

Two of the four judging axes — **Presentation** and **Business Value** — are
decided almost entirely by artifacts this plan produces. This plan runs **after**
DP-SUBMIT, because `deckgen` and `faqdef` consume `disclosure.md`.

### Prerequisite gate (state it as WU-PITCH-00)

`disclosure.md` and `architecture-summary.md` must already exist (DP-SUBMIT).
If they do not, stop and run DP-SUBMIT first — do not hand-write them.

### What your plan must own

**Deck → `deck/` → `deck/mockrill-deck.pdf`** (submission field 7, PDF mandatory):
```
...
```
Run `--no-llm` first — the offline output must be shippable on its own — then an
optional LLM polish pass. Specify the slide order and the one-sentence job of
each slide, and which judging axis it serves. At minimum: 01 title + the hook
line, 02 problem (the spoken-screen freeze), 03 specific user (junior bootcamp
grads / career switchers — named, never "everyone"), 04 the product in one
sentence, 05 business value (B2C subscription + B2B bootcamp career-services
licensing; TAM shown as a bottom-up estimate with its arithmetic visible),
06 how it works (the architecture diagram from `docs/architecture.mmd`),
07 AssemblyAI usage in detail (Universal-3.5-Pro streaming with the literal
parameters — `format_turns`, `keyterms_prompt`, `end_of_turn_confidence_threshold`,
`vad_threshold`, `interruption_delay` — plus LLM Gateway JSON-Schema tool calling),
08 originality / prior art, 09 the re-drill loop, 10 demo, 11 what's next,
12 disclosure + license. Export to PDF and state the exact export step.

**TAM honesty rule:** `winning_project_plan.md` carries `grounded: false` and
`tam_figure: [figure to pin down]`, and the `pgm`/`profile` modules are **excluded
from this working copy**, so there is no automated grounding available. **Do not
invent a market figure.** Specify a bottom-up estimate whose inputs are each
cited or labelled `[estimate]`, and show the arithmetic on the slide. A visibly
honest estimate beats an unsourced big number in front of judges who ask.

**Timed script → `script.md`** (submission field 6 support):
```
cp <a timing template> my-timing.yaml     # edit minutes only
...
```
Note that `examples/` fixtures are git-ignored in this repo, so the timing file is
authored fresh; specify its full contents. **Total must be 4:00** — inside the
3–5 minute rubric band, with a minute of margin at both ends. Give the per-section
budget in mm:ss and the rule that the video **opens on the ticking transcript,
not on a title slide** — the first 10 seconds decide the Presentation score.

**Judge Q&A → `docs/qa/qa-sheet.md`:**
```
...
```
Beyond the generated questions, your plan must specify **verbatim answers** to
these five, because they decide the close call:
1. *"Why is this AssemblyAI and not any STT?"* — word-level timestamps with
   `end_of_turn`/`turn_is_formatted` turn semantics are what make timestamped
   evidence and natural barge-in possible; the LLM Gateway keeps the whole stack
   on one key; name the literal socket parameters used.
2. *"You chose Path B — isn't Path A the 'real' one?"* — the brief lists both as
   equal; the hosted URL required by field 10 needs a serverless deployment,
   which cannot hold a long-lived socket; Path B puts more AssemblyAI surface
   (streaming + LLM Gateway) in the shipped product, not less.
3. *"How do I know the scorecard isn't hallucinated?"* — evidence quotes come
   from deterministic detection over `words[].start/end`, never from the LLM;
   `mergeScores` explicitly discards LLM-supplied quotes.
4. *"What's the business model / who exactly pays?"* — named user, B2C
   subscription, B2B bootcamp career-services seats, with the bottom-up numbers.
5. *"What's original here — there are other interview bots."* — the re-drill
   loop: quote the exact moment, then make the candidate say it again better in
   the same session, on the same socket. Name the overlapping prior art
   (evidence-backed voice agents with speaker+timestamp citations; generic
   interview-prep bots) and state the differentiator explicitly.

**DEMODRIVE capture → `assets/demodrive/`** (fallback-ladder rung 3):
```
npm run dev                                   # terminal 1
npm run mock:publish                          # terminal 2
...
```
Specify the full contents of `demodrive-script.json` (the click sequence through
Setup → LiveCall → ScorecardView → Drill). **Capture early — in week 3, not the
final week — and re-capture weekly.** State that it is insurance, and insurance
bought late is worthless.

**Cover image → `assets/cover.png`** (submission field 5): PNG, **16:9**,
1920×1080, derived from deck slide 01. Specify the exact dimensions and the check.

**`docs/video-shotlist.md`:** shot-by-shot with mm:ss marks matching `script.md`,
naming for each shot what is on screen and what is said. Include the rule that
the demo segment shows a **real spoken session**, and that the DEMODRIVE capture
is the substitute only if live capture fails on recording day.

### Mandatory work units (at least these)

0. `WU-PITCH-00` — verify `disclosure.md` + `architecture-summary.md` exist.
1. `WU-PITCH-01` — `deckgen populate --no-llm` → `deck/`.
2. `WU-PITCH-02` — the business-value slide with the bottom-up TAM arithmetic.
3. `WU-PITCH-03` — the AssemblyAI-usage slide with literal parameters.
4. `WU-PITCH-04` — `my-timing.yaml` (full contents) + `script validate` passing.
5. `WU-PITCH-05` — `script generate` → `script.md` at 4:00.
6. `WU-PITCH-06` — `faqdef generate` + the five verbatim answers.
7. `WU-PITCH-07` — `faqdef rehearse --count 5 --budget 90` twice.
8. `WU-PITCH-08` — `demodrive-script.json` + `validate`.
9. `WU-PITCH-09` — `demodrive capture` (week 3) + the weekly re-capture rule.
10. `WU-PITCH-10` — deck → PDF export and `assets/cover.png` at 1920×1080.
11. `WU-PITCH-11` — `docs/video-shotlist.md` + two rehearsals timed to ≤5:00.

Every work unit's verification is the literal command plus its exact expected
exit code and first output line, or — for the artifact checks — a one-line shell
assertion (e.g. an `identify`/`file` call proving the cover is 1920×1080).
