# PROMPT-DP-CONTRACTS — author the design plan for Mockrill's shared contracts
...
`private/design_documents/design_plans/DP-CONTRACTS.md`.

## YOUR PLAN: DP-CONTRACTS — Mockrill Shared Contracts & Event Vocabulary

This is the keystone plan. Every other plan imports from it and nothing in it
imports from any other Mockrill plan. It contains **types, constants and three
pure functions — no behavior, no network, no React, no scoring logic**.

### What your plan must own (rows M1–M15 of the table in S7 below)

`src/mockrill/contracts/types.ts` — M1 `TranscriptWord`, M2 `TranscriptTurn`,
M3 `InterviewQuestion`, M4 `RubricAxis`, M5 `EvidenceQuote`, M6 `FillerHit`,
M7 `AnswerScore`, M8 `Scorecard`, M9 `SessionState`.

`src/mockrill/contracts/steps.ts` — M10 `MOCKRILL_STEP_IDS` (a `readonly` tuple,
`as const`) and `MockrillStepId`; M11 `StepPayloads`, the mapped type from step
id to payload. Your plan must specify the payload shape for **every one of the
nine step ids** explicitly. Use exactly these, and define each field:

| step_id | status values it may carry | payload |
|---|---|---|
| `session-start` | `started` | `{ session_id: string; role: string; began_at: string }` |
| `mic-capture` | `started` \| `error` | `{ sample_rate: number; muted: boolean; error?: string }` |
| `transcript-partial` | `streaming` | `{ turn: TranscriptTurn }` |
| `transcript-final` | `done` | `{ turn: TranscriptTurn }` |
| `question-asked` | `started` \| `done` | `{ question: InterviewQuestion; spoken: string }` |
| `answer-scored` | `done` \| `error` | `{ score: AnswerScore }` |
| `scorecard-ready` | `done` | `{ scorecard: Scorecard }` |
| `drill-start` | `started` | `{ question_id: string; attempt: number }` |
| `session-end` | `done` \| `error` | `{ session_id: string; duration_ms: number; reason: string }` |

`src/mockrill/contracts/envelope.ts` — M12 `makeEnvelope`. Specify exactly: it
imports `EventEnvelope` **as a type** from `src/platform/transport`, fills
`timestamp` with `new Date().toISOString()`, uses a module-local monotone counter
for `sequence` when `opts.sequence` is absent, defaults `degraded` to `false`,
and never throws. Also specify `resetEnvelopeSequence()` (test seam) and state it
is internal-but-exported for tests only.

`src/mockrill/contracts/time.ts` — M13 `formatTimestamp(ms)`. Spell out the
algorithm literally: clamp negatives to 0, `Math.floor(ms/1000)`, minutes =
`Math.floor(total/60)`, seconds = `total % 60`, both zero-padded to 2 digits,
joined with `:`. Minutes are NOT capped at 59 (a 75-minute session renders
`75:04`). Give at least five worked examples including `0 → "00:00"`,
`462000 → "07:42"`, `-5 → "00:00"`, `59999 → "00:59"`, `3600000 → "60:00"`.

`src/mockrill/contracts/index.ts` — M14, the barrel. It re-exports M1–M13 and
nothing else. State the rule: all other Mockrill code imports from
`src/mockrill/contracts`, never from the individual files.

`engine/schema/input.schema.json` and `engine/schema/output.schema.json` — M15.
These currently hold `TODO(ENGINE)` stubs; your plan replaces them. Input schema
mirrors `TurnRequest` (owned by DP-INTERVIEWER — you define the JSON Schema
here because engine schemas are yours, and DP-INTERVIEWER's TS type must match
it field for field; say so explicitly and list the fields). Output schema mirrors
`InterviewerAction`. Both must be valid JSON Schema draft-07 objects with
`required` arrays and `additionalProperties: false`.

You also own the two `package.json` scripts `typecheck` (`tsc --noEmit`) and
`test:mockrill` (`vitest run tests/mockrill`), and the test directory
`tests/mockrill/` convention that every other plan will add files to.

### Design decisions you must make explicit in the plan

- All timestamps in `TranscriptWord.start/end` are **milliseconds from session
  start**, matching AssemblyAI's `Turn` message. State this in a comment
  requirement so no consumer divides by 1000.
- `AnswerScore.axes` values are integers 0–5 inclusive; `overall` is the
  arithmetic mean of the four axes rounded to one decimal. State the rounding
  rule literally (`Math.round(x * 10) / 10`).
- `Scorecard.overall` is the mean of `per_question[].overall`, same rounding;
  an empty `per_question` yields `overall: 0` and `weakest_question_id: null`.
- Nothing in `contracts/` may import from `src/mockrill/voice`,
  `src/mockrill/engine`, `src/mockrill/scoring` or `src/mockrill/ui`. The only
  non-relative import allowed anywhere in this plan is
  `import type { EventEnvelope } from "src/platform/transport";`.

### Mandatory work units (at least these, in this order)

1. `WU-CONTRACTS-01` — create `src/mockrill/contracts/types.ts` with M1–M9.
2. `WU-CONTRACTS-02` — create `src/mockrill/contracts/steps.ts` with M10–M11.
3. `WU-CONTRACTS-03` — create `src/mockrill/contracts/time.ts` with M13 + its unit test.
4. `WU-CONTRACTS-04` — create `src/mockrill/contracts/envelope.ts` with M12 + its unit test, importing the real chassis `EventEnvelope` type.
5. `WU-CONTRACTS-05` — create the barrel `src/mockrill/contracts/index.ts`.
6. `WU-CONTRACTS-06` — replace the two `engine/schema/*.json` stubs (M15).
7. `WU-CONTRACTS-07` — add the `typecheck` and `test:mockrill` scripts and create `tests/mockrill/.gitkeep`; verification runs both and both pass.

Each work unit's verification command must be a single runnable line. For
WU-CONTRACTS-04 the command must prove the chassis import resolves — e.g. a
`vite-node -e` one-liner that calls `makeEnvelope("transcript-final","done",{turn})`
and prints the resulting envelope's `step_id`, `status` and `sequence`, with the
exact expected stdout written out in the plan.
