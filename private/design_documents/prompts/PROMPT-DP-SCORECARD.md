# PROMPT-DP-SCORECARD — author the design plan for the evidence-backed scorecard
...
`private/design_documents/design_plans/DP-SCORECARD.md`.

## YOUR PLAN: DP-SCORECARD — Evidence-Backed Scoring & Re-Drill Selection

This plan owns the "wow" moment: *"at 07:42 you said 'kind of' 3× before
answering."* It is **pure, deterministic, offline** — no network, no LLM, no
React. Everything here is a pure function over `TranscriptTurn`s, which is what
makes the demo survive a dead network (fallback-ladder rung 2 and 4 both depend
on this plan working with zero connectivity).

### What your plan must own (rows M30–M36)

**M30 `src/mockrill/scoring/fillers.ts`.**
- `FILLER_LEXICON` — a frozen array. Write out the **complete literal list** in
  the plan; do not say "common fillers". Include at minimum: `um`, `uh`, `er`,
  `ah`, `like`, `kind of`, `sort of`, `you know`, `i mean`, `basically`,
  `actually`, `literally`, `right?`, `so yeah`, `i guess`, `whatever`, `stuff`,
  `things like that`. Mark which entries are **multi-word** — the matcher must
  handle phrases across adjacent `words[]` entries.
- `detectFillers(turn: TranscriptTurn): FillerHit[]` — numbered algorithm:
  1. Normalize each `word.text`: lowercase, strip leading/trailing punctuation
     `.,!?;:"'`, keep internal apostrophes.
  2. Sort the lexicon by descending word-count so multi-word phrases match first.
  3. Sliding window over the normalized token array; on a match emit
     `{ word: <lexicon entry>, start_ms: words[i].start, end_ms: words[j].end }`
     and advance `i` past the matched span (no overlapping matches).
  4. `like` is a filler ONLY when it is not preceded by `is`, `are`, `was`,
     `were`, `feel`, `felt`, `look`, `looks`, `sound`, `sounds`, `seem`, `seems`
     and not followed by `this`, `that`, `it`. Write the rule out; it is the
     difference between a credible demo and an embarrassing one.
  5. Never throw: a turn with `words: []` returns `[]`.

**M31 `src/mockrill/scoring/evidence.ts` → `buildEvidence(turn, hits)`.**
- Uses `formatTimestamp` from `src/mockrill/contracts` for `label` — **import
  it, never re-derive mm:ss.**
- Produces at most `MAX_EVIDENCE = 4` quotes per turn, chosen by this exact
  priority: (1) the highest-count filler cluster, (2) the longest silence gap
  ≥ `PAUSE_MS = 1500` computed as `words[i+1].start - words[i].end` → a `pause`
  quote, (3) the first sentence of the answer as a `quote` kind, (4) the lowest
  `confidence` word span as a `quote`.
- `note` is a fixed template per kind — write the three templates literally,
  e.g. filler: `` `said "${word}" ${count}× here` ``.
- `text` for a `pause` quote is the 5 words before and after the gap, joined.

**M32 `scoreAnswerDeterministic(turn, question)`.** Write the complete arithmetic.
Each axis is an integer 0–5 derived from measurable signals only:
- `structure`: STAR-marker coverage. Count how many of the four marker sets
  appear (situation: `when|at my|during|the project`; task: `i needed|my job|i was asked|the goal`;
  action: `i built|i wrote|i decided|i led|so i`; result: `resulted|we shipped|reduced|increased|the outcome`).
  Score = `markers * 1.25` rounded to the nearest integer, capped at 5.
- `specificity`: count of numerals, proper nouns (capitalized mid-sentence
  tokens) and technology keyterms from `question.keyterms` present in the
  transcript. 0 → 1, 1–2 → 2, 3–4 → 3, 5–6 → 4, 7+ → 5.
- `clarity`: starts at 5; subtract 1 per `FILLER_RATE_STEP = 0.02` of
  fillers-per-word above `FILLER_RATE_FLOOR = 0.02`; floor at 0.
- `relevance`: fraction of `question.keyterms` appearing in the transcript,
  mapped `0 → 1, ≤0.25 → 2, ≤0.5 → 3, ≤0.75 → 4, >0.75 → 5`.
Give at least three fully worked numeric examples with a literal transcript,
literal intermediate counts and the final `AnswerScore` object.
`overall` = mean of the four axes, `Math.round(x*10)/10`. `source: "deterministic"`.
`rationale` is built from a fixed template naming the weakest axis.

**M33 `mergeScores(llm, det)`.** Rules, stated as a table: when `llm` is `null`
return `det`. Otherwise per axis take `Math.round((llm+det)/2)`; `evidence` is
always `det.evidence` (the deterministic evidence is the timestamped, verifiable
one — the LLM's quotes are never trusted as evidence because it can hallucinate
the candidate's words); `rationale` prefers the LLM's; `source: "llm"`.
**Say explicitly why evidence never comes from the LLM** — this is the entry's
credibility argument and DP-PITCH will quote it in the Q&A sheet.

**M34 `buildScorecard(input)`** and **M35 `selectWeakest(scorecard)`.**
- `duration_ms` = last turn's last word `end` minus the first turn's first word
  `start`, floored at 0.
- `filter_total` / `filter_top`: total filler count and the 5 most frequent hits.
- `overall` = mean of `per_question[].overall`, same rounding; empty → 0.
- `selectWeakest` returns the `question_id` with the lowest `overall`; ties break
  by the earliest `turn_order`; an empty scorecard returns `null`.
- `degraded` propagates from the input.

**M36 `src/mockrill/scoring/index.ts`** — the barrel; the only import path other
plans use.

### Non-negotiables

- Zero imports outside `src/mockrill/contracts` and `src/resilience`
  (`isDegradedResult` only). No `fetch`, no `window`, no React, no Node-only APIs
  — this module must run identically in the browser and in vitest.
- Every function is total: no throws, no `undefined` returns, empty inputs give
  well-defined empty outputs.

### Mandatory work units (at least these)

1. `WU-SCORE-01` — `fillers.ts` (M30) with the full lexicon and the `like` rule.
2. `WU-SCORE-02` — `evidence.ts` (M31) with the priority ladder.
3. `WU-SCORE-03` — the four axis functions (M32), each with its worked example as a test case.
4. `WU-SCORE-04` — `mergeScores` (M33).
5. `WU-SCORE-05` — `buildScorecard` + `selectWeakest` (M34, M35).
6. `WU-SCORE-06` — the barrel (M36) and a cross-module verification that imports
   the real `formatTimestamp` from `src/mockrill/contracts` and prints the
   `label` of an evidence quote built from a literal fixture turn, with the exact
   expected stdout (e.g. `07:42`) written in the plan.

Every work unit's verification is a `vitest` run over a literal fixture written
into the plan, printing a specific asserted value. Because this plan is pure, its
tests are the entry's regression safety net — say so in §10.
