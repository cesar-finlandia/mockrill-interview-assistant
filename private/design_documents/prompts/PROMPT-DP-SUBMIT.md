# PROMPT-DP-SUBMIT — author the design plan for disclosure, hygiene and the 10-field submission
...
`private/design_documents/design_plans/DP-SUBMIT.md`.

## YOUR PLAN: DP-SUBMIT — Disclosure, Hygiene, License, Diagram & Submission Copy

This is a pass/fail plan. A brilliant demo with a missing MIT `LICENSE`, a
private repo, or a leaked key scores zero. It also produces `disclosure.md`,
which DP-PITCH consumes — so it runs **before** DP-PITCH.

### What your plan must own (rows M45, M46 and the artifacts below)

**Repo hygiene first — `WU-SUB-01` must be the first work unit in the plan.**
- Add `private/` to `.gitignore`. **The design blueprint and every design plan
  live under `private/design_documents/` and `.gitignore` does NOT currently
  cover it — without this line the entry's internal strategy documents ship in
  the public repo required by submission field 8.** State this as the reason.
- Confirm `.env`, `*.key`, `credentials*.json` and `.cache/` are already covered
  by the existing `.gitignore` (they are) and that no `.env` is tracked.
- Verification: `git check-ignore -v private/design_documents/master_blueprint_entry.md`
  prints the matching rule, and `git ls-files | grep -c '^\.env$'` prints `0`.

**MIT `LICENSE`** — the brief requires submissions to be *"original and
MIT-compliant"*. Specify the exact MIT text with the correct copyright year
(2026) and holder, and state that the year and holder are the only variable parts.

**`disclosure.md` + `architecture-summary.md`** — generated, never hand-written:
```
...
```
Specify: these must be regenerated after **any** change to
`winning_project_plan.md` or `assembly.manifest.json`, both are pure functions,
exit 0 on success. Also specify the `--ai-log` entries to pass (tool:scope pairs)
so AI-assistance disclosure is honest and specific rather than boilerplate.

The disclosure must additionally state, in the operator's own added section:
- which parts are **pre-built non-AI scaffolding** (the five included chassis
  modules: resilience, platform, context, ideation, provenance) and that they are
  the author's own open-source scaffolding, reused and disclosed;
- which parts were **built inside the Sep 1–30 window** (everything under
  `engine/`, `src/mockrill/`, `api/`, the prompts, the question bank, the
  dialogue policy, the scoring rubric);
- the deliberate technology choices: **Path B** with the browser connecting
  directly to AssemblyAI streaming, **AssemblyAI LLM Gateway** for orchestration,
  and **browser `speechSynthesis`** for voice output instead of a TTS vendor.
State that the Rule Book's explicit scaffold-disclosure clause was `[unconfirmed]`
at kickoff and that disclosing regardless is the safe choice.

**M45 `docs/architecture.mmd` → `docs/architecture.png`.** A Mermaid diagram
mirroring the architecture in §S4: browser box (mic → streaming client → turn
controller → speaker → event bus → screens), the AssemblyAI boxes (streaming WS,
token endpoint, LLM Gateway), the three serverless functions, and the offline
mock path. Specify the exact node labels and the render command. Also run
`npx vite-node src/ideation/deckgen/cli.ts diagram --manifest assembly.manifest.json --out docs/chassis-modules.mmd`
for the module-composition diagram and state that the two diagrams are
complementary (one is runtime data flow, one is module provenance).

**`README.md` — the spin-up guide.** Judges clone and run it. Specify the exact
sections and their content: one-paragraph what/why, a 90-second quickstart
(`npm ci`, copy `.env.example` to `.env`, set `ASSEMBLYAI_API_KEY`,
`npm run dev`), the hosted URL, the architecture diagram, how to run the offline
demo (`npm run preview` + `npm run mock:publish`), the AssemblyAI features used
with the exact parameter names (this is Application-of-Technology evidence a
judge reading only the repo will see), the license line, and a pointer to
`disclosure.md`. State that the README must not contain a key, a token, or a
personal transcript.

**M46 `submission.md` — the 10 fields.** Generated then hand-tightened:
```
...
```
Specify each of the 10 fields and its binding format:
1 title · 2 short description · 3 long description · 4 tech/category tags
(`AssemblyAI`, `Universal-3.5-Pro`, `Realtime STT`, `LLM Gateway`, `Voice Agent`,
`Interview Prep`) · 5 cover **PNG/JPG 16:9** · 6 video **MP4 3–5 min** ·
7 slides **PDF** · 8 public GitHub repo URL · 9 hosting platform ("Vercel") ·
10 Application URL. For each, name the producing plan (5–7 are DP-PITCH; 9–10 are
DP-DEPLOY) and the acceptance check.

**Hygiene gate:**
```
npx vite-node src/provenance/submit/cli.ts hygiene --manifest assembly.manifest.json --out hygiene-report
```
Note the exit-code contract: exit 1 on an explicit `flagged` verdict, and outputs
are still written. A flagged verdict blocks submission until resolved.

### The regeneration order (state it as a hard rule)

After ANY change to `winning_project_plan.md`, `assembly.manifest.json`, or the
deployed URL, re-run in this exact order:
`provo generate` → `provo summary` → `deckgen populate` → `script generate` →
`faqdef generate` → `submit format` → `submit hygiene`. They are pure functions;
running them out of order produces a `submission.md` that disagrees with
`disclosure.md`.

### Deadline discipline (put it in §10)

Official Wed 2026-09-30 15:00 UTC; internal target Wed 2026-09-24 15:00 UTC.
Manual submission is available only ≤6 h post-deadline with prior organizer
approval — never plan around it.

### Mandatory work units (at least these)

1. `WU-SUB-01` — `.gitignore` `private/` + secret audit (FIRST).
2. `WU-SUB-02` — MIT `LICENSE`.
3. `WU-SUB-03` — `provo generate` + `summary` + the added disclosure section.
4. `WU-SUB-04` — `docs/architecture.mmd` + PNG + the chassis-module diagram (M45).
5. `WU-SUB-05` — `README.md` spin-up guide.
6. `WU-SUB-06` — `submit format` → `submission.md` (M46), all 10 fields filled.
7. `WU-SUB-07` — `submit hygiene` green, with the exact expected verdict line.
8. `WU-SUB-08` — the regeneration-order runbook committed as `docs/repackage.md`.

Every work unit's verification is the literal CLI invocation plus its exact
expected exit code and the first line of its output.
