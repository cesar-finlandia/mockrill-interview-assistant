# PROMPT-DP-TURNTAKING — author the design plan for turn-taking, barge-in and voice output
...
`private/design_documents/design_plans/DP-TURNTAKING.md`.

## YOUR PLAN: DP-TURNTAKING — Turn-Taking, Barge-In & Voice Output

This plan owns the conversation's *timing*: when the interviewer speaks, when the
microphone is live, what happens when the candidate interrupts, and how the
session moves between states. It is the plan that makes the demo feel like a
phone screen rather than a form.

### What your plan must own (rows M21–M23, plus `engine/voice/policy.md`)

**M21 `src/mockrill/voice/speak.ts` → `createSpeaker()`.** Uses only
`window.speechSynthesis` — no TTS vendor, no second API key. Spell out:
1. `available` is `typeof window !== "undefined" && "speechSynthesis" in window`.
2. Voice selection: prefer, in order, the first voice whose `lang` starts with
   `en-` and whose `name` contains `Google`, then any `en-` voice, then
   `voices[0]`. Voices can load asynchronously — specify waiting on the
   `voiceschanged` event with a 1500 ms timeout before falling back.
3. `speak(text)` resolves on the utterance's `end` event and **also** resolves on
   `error` (it never rejects — the session must not stall on a TTS failure) and
   on a 30 s watchdog timeout.
4. `cancel()` calls `speechSynthesis.cancel()` and resolves any pending promise.
5. `rate: 1.05`, `pitch: 1.0`, `volume: 1.0` — literal values, justified as
   "slightly brisk, like a real screener".
6. When `available === false`, `speak()` resolves immediately and the caller is
   required to render the text on screen instead. State this contract.

**M22/M23 `src/mockrill/voice/turnController.ts`.** This is the state machine.
Specify it as an explicit table of transitions over `SessionState` (M9):

| from | event | to | side effects |
|---|---|---|---|
| `idle` | `start()` | `connecting` | emit `session-start` (started) |
| `connecting` | mic ready + socket open | `speaking` | emit `mic-capture` (started); ask question 1 |
| `connecting` | mic denied / socket degraded | `failed` | emit `mic-capture` (error) or `session-end` (error) |
| `speaking` | speaker finished | `listening` | `mic.setMuted(false)`; push `agent_context` via `UpdateConfiguration` |
| `speaking` | partial turn arrives with ≥ `BARGE_IN_MIN_WORDS` words | `listening` | `speaker.cancel()`; `mic.setMuted(false)` (barge-in) |
| `listening` | `onFinal` turn | `thinking` | `mic.setMuted(true)`; emit `transcript-final`; call `nextAction` |
| `thinking` | action with `question` | `speaking` | emit `question-asked`; `speaker.speak(action.say)` |
| `thinking` | action with `done: true` | `scoring` | emit `answer-scored` for the last score |
| `scoring` | scorecard built | `complete` | emit `scorecard-ready`; `client.terminate()` |
| any | `stop()` | `complete` | `speaker.cancel()`; `mic.stop()`; `client.terminate()`; emit `session-end` |
| any | unrecoverable degraded | `failed` | emit `session-end` (error, `degraded: true`) |

Write out every literal constant: `BARGE_IN_MIN_WORDS = 3`,
`MAX_QUESTIONS = 4`, `MAX_SESSION_MS = 900000` (15 min hard stop),
`THINKING_TIMEOUT_MS = 12000` (after which a canned bridging line —
"Let me follow up on that." — is spoken and the session continues degraded).

**Mute discipline (the single most important rule in this plan):** the mic is
muted for the entire duration of `speaking` so the agent never transcribes its
own synthesized voice. Barge-in is detected from partial turns that arrive
*despite* muting only when the browser's echo cancellation lets speech through;
therefore also specify the alternative barge-in trigger: a `mic-capture` chunk
whose peak amplitude exceeds `BARGE_IN_PEAK = 0.15` for 3 consecutive chunks
while `speaking`. Give the amplitude computation as numbered steps.

**`drill(questionId)`** — the differentiating feature. Steps: emit `drill-start`
(`attempt` = previous attempts + 1); speak a fixed framing line built from the
question text and the weakest axis; transition to `speaking` → `listening`
**without reconnecting the socket** (state this explicitly — the socket stays
open, which is what makes "re-drill in the same session" true); score the new
answer with the same path; replace the previous `AnswerScore` for that question
in the session state.

**`engine/voice/policy.md`** — replaces the `engine/voice/policy.todo.md` stub.
It is prose documenting the above policy for judges and for the Q&A sheet: turn
end detection, barge-in rule, mute discipline, and the latency budget.

### Requirements to trace

- FR-04 (speak, mute, barge-in), FR-09 (re-drill in the same session),
  NFR-06 (`end_of_turn` → interviewer starts speaking ≤ 2.0 s p50),
  NFR-02 (nothing throws to the UI).
- Specify how NFR-06 is measured: `performance.now()` at `onFinal` and at
  `speak()` start, logged into the `question-asked` envelope payload as
  `latency_ms`, so the number is demonstrable on stage.

### Mandatory work units (at least these)

1. `WU-TURN-01` — `createSpeaker` (M21) with the voice-selection and never-reject rules.
2. `WU-TURN-02` — the state machine skeleton and transition table (M22), pure and testable with injected fakes for mic/client/speaker.
3. `WU-TURN-03` — mute discipline + both barge-in triggers.
4. `WU-TURN-04` — `nextAction` wiring and the `THINKING_TIMEOUT_MS` bridge line.
5. `WU-TURN-05` — `drill()` re-entry without reconnecting.
6. `WU-TURN-06` — `latency_ms` instrumentation.
7. `WU-TURN-07` — `engine/voice/policy.md`.

At least one work unit's verification command must drive the real state machine
through `idle → connecting → speaking → listening → thinking → speaking` with
fake dependencies and print the visited states, with the exact expected stdout
written in the plan. At least one must import the real `SessionState` type from
`src/mockrill/contracts` and the real `createStreamingClient` type from
`src/mockrill/voice/streamingClient` to prove the cross-module contract holds.
